...
export default function Home() {
  ...
  if (isLoggedIn && user) {
    return (
      <DashboardShell contentClassName="p-6 lg:p-8 overflow-y-auto space-y-6">
...
function StatCard({ title, value, trend, trendDesc, trendUp, icon, color }) {
  ...
  return (
    <div className="bg-[#11141d] p-6 rounded-2xl border border-zinc-800 hover:border-zinc-700 transition-colors flex flex-col justify-between">
      ...
      <div
        className={`mt-4 h-1 w-full rounded-full bg-gradient-to-r opacity-50 ${
          color === "purple"
...
function ActionCard({ title, desc, icon, color, href }) {
  ...
  const getColorClasses = () => {
    switch (color) {
      case "blue":
        return "bg-[#1a2344] text-blue-400 group-hover:bg-blue-500/20 group-hover:shadow-[0_0_15px_rgba(59,130,246,0.3)]";
      case "purple":
        return "bg-[#2a2454] text-purple-400 group-hover:bg-purple-500/20 group-hover:shadow-[0_0_15px_rgba(168,85,247,0.3)]";
      case "green":
        return "bg-[#1a2e28] text-green-400 group-hover:bg-green-500/20 group-hover:shadow-[0_0_15px_rgba(34,197,94,0.3)]";
      case "orange":
        return "bg-[#30211a] text-orange-400 group-hover:bg-orange-500/20 group-hover:shadow-[0_0_15px_rgba(249,115,22,0.3)]";
      default:
  ...
  const className =
    "flex flex-col items-center justify-center p-6 bg-[#1a1f2e] rounded-xl border border-zinc-700/50 hover:border-zinc-600 transition-all hover:-translate-y-1 group text-center h-full";
  const inner = (
    <>
      ...
      <main className="relative z-10 flex flex-col lg:flex-row items-center justify-center flex-1 max-w-7xl mx-auto px-6 py-12 gap-12 w-full">
        <div className="flex flex-col items-start text-left lg:w-1/2">
          ...
          <p className="text-lg text-gray-400 mb-10 max-w-xl">
            Welcome to your AI-powered multi-agent business platform. Automate workflows, extract
            insights, and deploy autonomous agents with a stunning, intuitive OS interface.
          </p>
